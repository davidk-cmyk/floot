...
export async function handle(request: Request) {
  try {
    ...
    const existingUsers = await db
      .selectFrom("users")
      .select(["id", "email", "displayName", "role", "hasLoggedIn"])
    ...
    if (existingUsers.length > 0) {
      ...
      // Check if this is the user's first login via OAuth
      ...
      // If it's the first login, update login tracking fields
      if (isFirstLogin) {
        updateData.hasLoggedIn = true;
      ...
      // Determine organization for new OAuth user
      // First try to find organization by email domain
      const emailDomain = mappedUserData.email.split("@")[1];
      ...
      if (emailDomain) {
        ...
        if (orgByDomain.length > 0) {
          ...
          console.log(
            `Found organization ${organizationId} for domain ${emailDomain}`
          );
        }
      }

      // If no organization found by domain, try to get default organization
      if (organizationId === null) {
        ...
        if (defaultOrg.length > 0) {
          ...
          console.log(
            `Using default organization ${organizationId} for new OAuth user`
          );
        }
      }

      // If still no organization found, fail with clear error
      if (organizationId === null) {
        console.error(
          "Cannot create OAuth user - no organization found for domain and no default organization exists"
        );
        ...
        const errorMessage: OAuthErrorMessage = {
          ...
          error: {
            message:
              "Unable to create account. Please contact your administrator for access.",
            code: "no_organization",
            details: `No organization found for domain ${emailDomain}`,
          },
        };
        return new Response(createOAuthPopupResponseHtml(errorMessage), {
          status: 400,
      ...
      // Create new user with first login tracking
