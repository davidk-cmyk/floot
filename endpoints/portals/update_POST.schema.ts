...
export const schema = z
  .object({
    ...
    slug: z
      ...
      .optional(),
    label: z.string().max(100, "Label must be at most 100 characters.").nullable().optional(),
    description: z.string().optional(),
    accessType: PortalAccessTypeSchema.optional(),
    password: z.string().nullable().optional(), // Allow null to clear password
    allowedRoles: z.array(z.enum(UserRoleArrayValues)).optional(),
    isActive: z.boolean().optional(),
    requiresAcknowledgment: z.boolean().optional(),
    acknowledgmentDueDays: z.number().int().min(1).nullable().optional(),
    acknowledgmentReminderDays: z.number().int().min(1).nullable().optional(),
...
export const postUpdatePortal = async (
  body: InputType,
  init?: RequestInit
): Promise<OutputType> => {
  const validatedInput = schema.parse(body);
  const result = await fetch(`/_api/portals/update`, {
    method: "POST",
