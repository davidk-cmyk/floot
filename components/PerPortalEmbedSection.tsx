...
import styles from "./PerPortalEmbedSection.module.css";

type PerPortalEmbedSectionProps = {
  portalSlug: string;
  portalName: string;
...
export const PerPortalEmbedSection = ({
  portalSlug,
  portalName,
  organizationId,
  customDomain,
  className,
}: PerPortalEmbedSectionProps) => {
  ...
  const embedCode = useMemo(() => {
    ...
    if (customDomain && customDomain.trim()) {
      embedUrl = `https://${customDomain}/${organizationId}/${portalSlug}`;
    } else if (isProduction) {
      embedUrl = `${window.location.origin}/${organizationId}/${portalSlug}`;
    } else {
      embedUrl = `https://your-domain.com/${organizationId}/${portalSlug}`;
    }
  ...
  <p>Your browser does not support iframes. <a href="${embedUrl}">Visit our ${portalName} portal directly</a>.</p>
</iframe>`;
  }, [customDomain, portalSlug, portalName, organizationId, embedWidth, embedHeight, embedBorder]);
  ...
  return (
    <div className={`${styles.formGroup} ${className ?? ""}`}>
      <label className={styles.label}>Embed "{portalName}" Portal</label>
      <div className={styles.embedSection}>
        <p className={styles.embedDescription}>
          Generate HTML code to embed the "{portalName}" portal directly into
          your website. Visitors can access this specific portal without leaving your site.
        </p>
        ...
        <div className={styles.embedInstructions}>
          ...
          <ul className={styles.embedInstructionsList}>
            ...
            <li>
              Visitors can access the "{portalName}" portal and its policies directly from
              your site
            </li>
            <li>
              The embed respects the portal's access settings (public, password-protected, etc.)
            </li>
            {window.location.hostname === "localhost" && (
              <li className={styles.developmentNote}>
                <strong>Development Note:</strong> In production, this will use
                your actual domain. Configure a custom domain in organization settings for branded
                embedding.
              </li>
            )}
          </ul>
