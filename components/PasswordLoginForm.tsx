...
export const PasswordLoginForm: React.FC<PasswordLoginFormProps> = ({
  ...
  useEffect(() => {
    if (authState.type === "authenticated" && organizationState.type === "active") {
      ...
      setTimeout(() => navigate(`/${orgId}/admin/dashboard`), 200);
    } else if (authState.type === "authenticated" && organizationState.type === "no-organization") {
      setTimeout(() => navigate("/organizations"), 200);
    }
  ...
  const handleSubmit = async (data: LoginFormData) => {
    ...
    try {
      const loginData = organizationId 
        ? { ...data, organizationId }
        : data;
      const result = await postLogin(loginData);
      onLogin(result.user, result.isFirstLogin);
    } catch (err) {
