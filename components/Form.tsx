...
import React, { Dispatch, SetStateAction, useRef, useEffect, useMemo } from "react";
...
export function useForm<T extends z.ZodType<any, any>>({
  ...
  // Deep equality check for errors to prevent unnecessary re-renders
  ...
  // Computed property to check if form is valid without side effects
  const isValid = useMemo(() => {
    ...
    try {
      ...
      if (err instanceof ZodError) {
        // Check if there are only extra property errors (which should throw)
        ...
        // If there are only extra property errors, this will throw in validateForm
        // but for isValid check, we consider it invalid
        if (extraPropertyErrors.length > 0) return false;
      }
      return false;
    }
  }, [schema, values]);
  ...
  }, [schema, values, errors, deepEqual]);
  ...
  // Update the ref whenever validateField changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    for (const k of diffKeys) {
      ...
      validateFieldRef.current(k, { shallow: true });
    }
  }, [values]);
